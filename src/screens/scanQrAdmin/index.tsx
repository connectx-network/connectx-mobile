...
import {CONFIRM_INFO_USER_SCREEN} from '@navigation/routes';
...
const ScanQrAdminScreen = ({navigation}) => {
  ...
  //   setDeviceCamera(prev => (prev === 'back' ? 'front' : 'back'));
  // }, []);
  ...
  const onCameraRead = useCallback(async ({data}: BarCodeReadEvent) => {
    ...
    if (eventId && userId) {
      return navigate(CONFIRM_INFO_USER_SCREEN, {eventId, userId});
    }
        ...
        <Block style={styles.headerOverlay}>
          <Block style={[styles.rowOption, {paddingTop: top + getSize.m(15)}]}>
            ...
            <Text style={styles.textNoteScan}>
              Please scan into the QR code user provided
            </Text>
          </Block>
        </Block>
        <Block row>
...
export default ScanQrAdminScreen;
